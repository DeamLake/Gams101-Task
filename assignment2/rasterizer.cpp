...
{   
    ...
    Vector2f ca(_v[0].x()-_v[2].x(),_v[0].y()-_v[2].y());
    Vector2f ap(x-_v[0].x(),y-_v[0].y());
    Vector2f bp(x-_v[1].x(),y-_v[1].y());
    Vector2f cp(x-_v[2].x(),y-_v[2].y());

    float abp = ab.x() * ap.y() - ab.y() * ap.x();
    float bcp = bc.x() * bp.y() - bc.y() * bp.x();
    float cap = ca.x() * cp.y() - ca.y() * cp.x();
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    float down = std::min({v[0].y(),v[1].y(),v[2].y()});
